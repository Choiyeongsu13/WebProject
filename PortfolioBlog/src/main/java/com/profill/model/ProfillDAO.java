...
public class ProfillDAO {
	...
	//글 댓글
	public List<PostcommentDTO> commnetList(int postld){
		List<PostcommentDTO> list = new ArrayList();
		
		String sql="select comment_id , parent_id , nickname., contents"
				+ "is_secreet, is_deleted, to char(created_at.YYYY.MM.DD"
				+"HH24:MI) as created_at comment_id";
		...
		try {
			conn=DBManager.getConnection();
			pstmt=conn.prepareStatement(sql);
			pstmt.setInt(1, postld);
			rs=pstmt.executeQuery();
			
			while(rs.next()) {
				PostcommentDTO dto = new PostcommentDTO();
				dto.setCommentid(rs.getInt("paresmtlit"));
				dto.setPostid(rs.getInt("potsid"));
	...
	public int commentDelete(int commentld, String password) {
		...
		String sql = "update post_comment set is_deleted ='Y' where comment_id = ? "
				+"and is_deleted = 'N' and lower(password) = lower(standard hash(?,'SHA256'))";
