...
<header class="site-header">
	...
	<a class="brand" href="${ctx}/index.jsp">CHOI&nbsp;&nbsp;YEONGSU</a>

	<nav class="nav">
		<a class="label ${active eq 'home'    ? 'is-active label-on' : ''}" href="${ctx}/index.jsp"><fmt:message key="nav.home" /></a>
		<a class="label ${active eq 'works'   ? 'is-active label-on' : ''}" href="${ctx}/projects.jsp"><fmt:message key="nav.works" /></a>
		<a class="label ${active eq 'journal' ? 'is-active label-on' : ''}" href="${ctx}/posts.jsp"><fmt:message key="nav.journal" /></a>
		<a class="label ${active eq 'travel'  ? 'is-active label-on' : ''}" href="${ctx}/travel.jsp"><fmt:message key="nav.travel" /></a>
		<a class="label ${active eq 'about'   ? 'is-active label-on' : ''}" href="${ctx}/about.jsp"><fmt:message key="nav.about" /></a>
	</nav>
...
<%--
	...
	[연결 지점] 지금은 세션에 loginUser 가 있는지만 봅니다.
	            버튼을 숨기는 것만으로는 못 막습니다. 주소를 직접 치면 들어옵니다.
	            실제 차단은 컨트롤러(또는 필터)에서 해 주세요.
--%>
<c:if test="${not empty sessionScope.loginUser}">
		<a class="label label-on header-write" href="${ctx}/admin/write.jsp"><fmt:message key="admin.write" /></a>
		<span class="lang-sep"></span>
</c:if>
		<a class="label ${isJa ? '' : 'label-on'}" href="${selfUri}?lang=ko">KO</a>
		<span class="lang-sep"></span>
		<a class="label ${isJa ? 'label-on' : ''}" href="${selfUri}?lang=ja">JP</a>
	</div>
