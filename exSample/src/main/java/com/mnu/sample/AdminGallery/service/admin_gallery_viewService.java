...
public class admin_gallery_viewService implements Action {
	...
	public void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Agdao.AdminGalleryCount(idx); //조회수 +1
		AdminGalleryDTO Agdto = Agdao.AdminGallerySearch(idx);

		Agdto.setContents(Agdto.getContents().replace("\n","<br>"));
		request.setAttribute("dto", Agdto);
		RequestDispatcher rd = request.getRequestDispatcher("/Admin/gallery_view.jsp");
