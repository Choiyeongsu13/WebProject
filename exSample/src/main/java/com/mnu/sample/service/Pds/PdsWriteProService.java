...
public class PdsWriteProService implements Action {
	...
	public void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//사용자가 보낸 HTTP(request) 를분석하여 요청 안에 들어 있는 모든 폼 데이터와 파일들을 FileItem 객체들로 바꾸어 리스트 형태로 반환
		List items = upload.parseRequest(request);
		
		//리스트에 담긴 데이터들을 하나씩 꺼내어 확인하기 위해 반복자(iterator) 를준비
		...
		FileItem file = (FileItem)iter.next(); //경로를 제외한 이름
		...
		response.sendRedirect("/Pds?cmd=pds_list");
	}
